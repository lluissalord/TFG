...
#include <dynamic_movement_primitive_controller/variable_name_map.h>
...
template<class DMPType>
  ...
  public:
     ...
     * @return
     */
    bool getDMP(typename DMPType::DMPPtr& dmp);
  ...
  /*private:*/
    ...
    geometry_msgs::PoseStamped::ConstPtr goal_pose_ptr_;
...
template<class DMPType>
                                                        ...
                                                        const std::vector<std::string>&dmp_variable_names)
  {
    ...
    ROS_VERIFY(dmp_filtered_subscriber_.initialize(100, node_handle, "Aibo_execute/dmp"/*controller_name + "/command"*/, boost::bind(&DMPControllerImplementation<DMPType>::filter, this, _1, _2)));
    ROS_VERIFY(dmp_goal_subscriber_.initialize(100, node_handle, "/pos"));//controller_name + "/goal"));
...
// REAL-TIME REQUIREMENTS
template<class DMPType>
  bool DMPControllerImplementation<DMPType>::getDMP(typename DMPType::DMPPtr& dmp)
  {
    ROS_ASSERT(initialized_);
    dmp = static_cast<typename DMPType::DMPPtr>(dmp_);
    return true;
  }
...
template<class DMPType>
  ...
  {
    //ROS_INFO("no ha entrat a set %i %i %i %i", dmp->isSetup(), dmp->isStartSet(), dmp->setup(), true);
    if (dmp->isSetup())
    {
      ROS_INFO("ha entrat a set");
...
template<class DMPType>
  bool DMPControllerImplementation<DMPType>::changeGoal()
  {
    ...
    if (goal_pose_ptr_)
    {
      if(goal_pose_ptr_->pose.orientation.x>0)
      {
        ROS_INFO("Change GOAL");
        ROS_VERIFY(dmp_->changeGoal(0, 0));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 1));
        ROS_VERIFY(dmp_->changeGoal(-goal_pose_ptr_->pose.orientation.x, 2));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 3));
        ROS_VERIFY(dmp_->changeGoal(0, 4));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 5));
        ROS_VERIFY(dmp_->changeGoal(-goal_pose_ptr_->pose.orientation.x, 6));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 7));
      }
      else
      {
        ROS_INFO("Change GOAL");
        ROS_VERIFY(dmp_->changeGoal(goal_pose_ptr_->pose.orientation.x, 0));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 1));
        ROS_VERIFY(dmp_->changeGoal(0, 2));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 3));
        ROS_VERIFY(dmp_->changeGoal(goal_pose_ptr_->pose.orientation.x, 4));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 5));
        ROS_VERIFY(dmp_->changeGoal(0, 6));
        ROS_VERIFY(dmp_->changeGoal(abs(goal_pose_ptr_->pose.orientation.y), 7));
      }
/*      for (int i = 0; i < num_variables_used_; ++i)
      {
...
template<class DMPType>  
  bool DMPControllerImplementation<DMPType>::readPos()
  {
    goal_pose_ptr_ = dmp_goal_subscriber_.poll();
    if(goal_pose_ptr_)
    {
      goal_pose_ = *goal_pose_ptr_;
      return true;
    }
    else
    {
