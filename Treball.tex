...
\fancyhead[RE,LO]{Equilibri del robot AIBO utilitzant DMPs}
\fancyfoot{}
...
\section*{Motivació}
\addcontentsline{toc}{section}{Motivació}
\label{Motivacio}
...
\addcontentsline{toc}{section}{Definició del \textit{TFG}}
...
\addcontentsline{toc}{section}{Requeriments previs}
\label{Requeriments}
...
%No tant concret i dir el perque de cada una de les coses, en plan, no dir ROS pero si un entorn que permeti la comunicacio i que sigui capdavantera i molt utilitzada com es ROS ara que es un dels entorns de treball més utilitzats en robotica i dels més extesos mundialment. Després la pendent sigui apta per estar en un edifici adaptat (com que la pendent es de 12% doncs si pot pujar-la ja esta). Temps de resposta correponent al temps que tardaria entrar en la pendent. DMPs i RL es realment utilitzar algorismes que siguin estat de l'art.
...
\section{Metodologia}
\label{Metodologia}
%Explicar que s'ha anat quedant periodicament amb els tutors i cotutors, després també s'ha treballat en un laboratori i varis companys que també estan fent PFC i algun TFG. També ens hem trobat amb el Ricardo Téllez creador original del package de ROS per AIBO.
...
\label{Abast}
%Dir que és un TFG i són 12 credits ECTS amb una indicacio orientativa d'unes 25 hores per credit, segons NORMATIVA DE TREBALLS DE FI DE GRAU, per tant a unes 300 hores.
%En aquest temps el que es pot arribar a aconseguir es limitat, al ser un treball que busca utilitzar algorismes estat de l'art s'ha de tenir en compte que poden ser de dificil enteniment i implementacio.
...
%MIRAR SI ES CERT QUE ÉS ES QUINT VALOR
...
%POSAR EN ANNEXOS COM INSTAL·LAR I EXECUTAR EL ROSSERIAL o posar-ho en apartat de funcionament
...
\begin{figure}[bt]
\centering
\resizebox{\textwidth}{!}{\input{Imatges/diagrama-sniekum.pdf_tex}}
...
\begin{figure}[bt]
\centering
\resizebox{\textwidth}{!}{\input{Imatges/dibujo.pdf_tex}}
\caption{Procés iteratiu del codi Policy$\_$learning}
\label{fig:iteracio-policy-learning}
\end{figure}


%MEDITAR SI POSAR LO DE SES MODIFICACIONS O NO!!!
...
%SI ES VOL ES POT EXPLICAR ES TEMA DE ARXIUS .STL o .DAE, diferencies, explicar com seria es tema de posar les unions en els punts adequats
...
%Explicar quins són els goals de les DMPs
%Com es plantejaria la funció recompensa
...
%REFERENCIA A APUNTS D'UNA ASSIGNATURA???
...
%MIRAR SI ES MEU AIBO ES ES 124 o 125.
...
%SI ES FA LA CONNEXIO DE LA PLATAFORMA POSAR COM ES CONNECTA

\section{Processos previs}

%TREURE LA PLATAFORMA SI ES QUE NO ES CONNECTA
%TREURE LO D'EXPLICAT LA INSTAL·LACIO SI CAL
...
%Possible implementacio del PI2, només sobre les articulacions del genoll o només sobre les que ja es desplacen ara.
...
%Posar que el fet de fer ús dels algorismes consumeix més energia?
