...
\label{Modelat i algorismes d'aprenentatge}
\subsubsection*{(2) Modelat i algorismes d'aprenentatge}
\addcontentsline{toc}{subsubsection}{Modelat i algorismes d'aprenentatge}
...
\paragraph{}Per crear el model d'un robot existeixen diverses possibilitats. La més rudimentària és prenent les mesures del propi robot i introduir-les al programa, aquest mètode avui dia és molt poc utilitzada. El més típic és utilitzar el propi robot per fer el model, on a través d'un sistema realimentat amb (1) robot, (2) el model en construcció i (3) un controlador per la realimentació; s'arriba finalment a desenvolupar el model.

\paragraph{}Ara bé, existeixen tant diferents tipus de models com també formes diferents de crear-los:
...
\item[Indirectes]
\item[Mixtes]
\item[De predicció de múltiples passos]
\end{description}
\end{itemize}
...
%Tipus de modelats(abans dir que no s'utilitza el fet de fer el model directament a mà, sino que s'extreu a partir del propi aprenentatge del robot)
